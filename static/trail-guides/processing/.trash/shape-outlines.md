...
:::quick-take{src="shape-outlines.png"}
Use the `stroke()` and `fill()` methods to set the outline and fill colors of subsequent shapes. The computer will keep these color settings for all the shapes it draws afterward until you have it execute `stroke()` or `fill()` again with different parameters.

```java
void setup() {
	size(400, 400)
	fill(#964B00);  // you can use single numbers, rgb numbers, or hex for these
	stroke(#5C4033);  
	rect(190, 150, 20, 150);
}
```
