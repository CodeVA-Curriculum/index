...
short: Learn how to create simple animations using the `draw()` and the Processing animation loop.
long: In this tutorial, we'll cover how to create an **animation loop** using the Processing `draw()` method. The introduction of the `draw()` method represents a significant change in how you might use Java Processing to create programs. With the `draw()` method, static images will become animations.
video: https://youtube.com/embed/Mu8VDPhdPdw
type: tutorial
---

:::quick-take
Processing includes another "lifecycle method" like `setup()` called `draw()`. The computer executes the code inside the `draw()` method definition **in a loop, forever, about once every 1/60th of a second**. This allows you to program the computer to display moving animations:
