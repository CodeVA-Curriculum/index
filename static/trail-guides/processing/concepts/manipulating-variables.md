...
:::quick-take
Once the computer creates a variable, it keeps its name but it can change the *value* of that variable later on if instructed. Use **arithmetic operations** to modify number values relative to themselves:
...
void setup() {
	size(100, 100);
	circle(x, 50, 25);
	x = x + 15; // take the value of x, add 10 to it, then replace the value of x with that new value
	circle(x, 50, 25);
	x += 15 // this is the same as above, just shorter
	circle(x, 50, 25)
}
```
:::
