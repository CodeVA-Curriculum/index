...
:::quick-take{src="background-color.png}
Pass **arguments** to the `background()` method to set the color the computer will use to cover the canvas. You can notate it a couple different ways:

```java
void setup() {
	background(100); // grayscale
	background(255, 0, 0); // rgb
	background(#00ff88); // hex code
}
```
:::
