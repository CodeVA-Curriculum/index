...
:::quick-take{src="circle.png"}
Use the `circle(x,y,d)` method to have the computer display a circle. The `x` and `y` parameters set the position of the circle on the coordinate plane, and the `d` parameter sets the diameter of the circle:
...
The `circle()` method is the easiest way to create a circle.  The program below shows that the `circle()` method takes three arguments:  circle center x-position, circle center y-position, and circle diameter.
