...
short: Learn how to insert images into your Processing sketches using the `loadImage()` and `image()` methods, along with the basics of how to define objects in Java.
long: In this tutorial, you'll learn how to insert image files into your Processing programs. Having the computer display whole images rather than just simple shapes is very useful, especially for making games and other projects that require intricate animations.
video: https://youtube.com/embed/Mu8VDPhdPdw
type: tutorial
---

:::quick-take
1. First, find an image to use and save it in the same location as your Processing program
2. Declare a variable to store the image data, then use `loadImage()` to initialize the image object during the `setup()` method.
3. Use the `image(obj, x, y)` method to have the computer display the image at the given position
...
:::prompt{title="Image Attribution"}
Create a repository of at least five images and associated references that you could use in your Java Processing programs.  Be sure that the image provides the appropriate Creative Commons attribution license for you to use the image.

You can download an example by clicking [here](TODO:).
:::