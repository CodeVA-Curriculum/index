...
long: In this tutorial, you'll learn how to use various methods to set and modify the shape outline and fill colors Processing displays when it executes shape display commands. These methods are essential if you want to use shape primitives that look different from Processing's default white.
---

:::quick-take{src="TODO:"}
Use the `stroke()`, `fill()`, `noStroke()`, and `strokeWeight()` methods to modify the colors of shapes' outlines and infills:

```java
void setup() {
	size(400, 400);
	stroke(0); // set stroke color to black
	strokeWeight(4); // set stroke weight to 2px
	fill(#ff0000); // set fill color to red
	circle(200, 200, 100);
	noStroke(); // turn off shape outline
